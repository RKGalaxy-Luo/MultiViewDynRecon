...
namespace SparseSurfelFusion {
	 ...
	 */
	__host__ __device__ __forceinline__
		float squared_distance(const float4& v3, const float4& v4) {
	...
	/**
	 * \brief 重载“-”：float4向量0 - float3向量1
	 * \param vec_0 输入向量0
	 * \param vec_1 输入向量1
	 * \return 返回vec_0 - vec_1
	 */
	__host__ __device__ __forceinline__ float3 operator-(const float4& vec_0, const float3& vec_1) {
		return make_float3(vec_0.x - vec_1.x, vec_0.y - vec_1.y, vec_0.z - vec_1.z);
	}

	/**
	 * \brief 重载“-”：float3向量0 - float4向量1
	 * \param vec_0 输入向量0
	 * \param vec_1 输入向量1
	 * \return 返回vec_0 - vec_1
	 */
	__host__ __device__ __forceinline__ float3 operator-(const float3& vec_0, const float4& vec_1) {
		return make_float3(vec_0.x - vec_1.x, vec_0.y - vec_1.y, vec_0.z - vec_1.z);
	}
	 ...
	 * \return 输出v1·v2
	 */
	__host__ __device__ __forceinline__ float dotxyz(const float3& v1, const float3& v2)
	{
		return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
	}
	...
	/**
	 * \brief 4维向量叉乘4维向量的x,y,z分量
	 * \param v1 输入4维向量
	 * \param v2 输入4维向量
	 * \return 输出4维向量叉乘4维向量的x,y,z分量的结果
	 */
	__host__ __device__ __forceinline__ float3 cross_xyz(const float4& v1, const float4& v2)
	{
		return make_float3(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x);
	}
	 ...
	 * \return 输出4维向量叉乘4维向量的x,y,z分量的结果
	 */
	__host__ __device__ __forceinline__ float3 cross_xyz(const float3& v1, const float3& v2)
	{
		return make_float3(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x);
	}
	 ...
	 * \return 返回向量的L1 Error
	 */
	__host__ __device__ __forceinline__ float fabsf_diff_xyz(const float4& vec_0, const float4& vec_1) {
		return fabsf(vec_0.x - vec_1.x) + fabsf(vec_0.y - vec_1.y) + fabsf(vec_0.z - vec_1.z);
	}
	 ...
	 * \return 返回向量的L1 Error
	 */
	__host__ __device__ __forceinline__ float fabsf_diff_xyz(const float3& vec_0, const float3& vec_1) {
		return fabsf(vec_0.x - vec_1.x) + fabsf(vec_0.y - vec_1.y) + fabsf(vec_0.z - vec_1.z);
	}
}
