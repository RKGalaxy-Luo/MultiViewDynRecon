...
namespace SparseSurfelFusion{
	...
	public:
		...
		DeviceBufferArray(const DeviceBufferArray&) = delete;                 
		DeviceBufferArray& operator=(const DeviceBufferArray&) = delete;      
		DeviceBufferArray(DeviceBufferArray&&) = delete;                      
		DeviceBufferArray& operator=(DeviceBufferArray&&) = delete;
		...
		//hsg
		DeviceArrayHandle<T> ArraySlice() { return DeviceArrayHandle<T>(array.ptr(), array.size()); }
		//hsg
		...
		void ResizeArrayOrException(size_t size) {
			if (size > buffer.size()) {
				printf("预计分配 Resize 大小 = %lld   实际开辟 Buffer 大小 = %lld\n", size, buffer.size());
