...
void SparseSurfelFusion::createUnsignedTextureSurface(const unsigned int rows, const unsigned int cols, cudaTextureObject_t& texture, cudaSurfaceObject_t& surface, cudaArray_t& cudaArray)
{
    //纹理描述
    cudaTextureDesc unsigned_texture_desc;
    createDefault2DTextureDescriptor(unsigned_texture_desc);
    //创建通道描述，使用指定类型返回通道描述子，并填入每一个通道分量的bit数 (下述为1个通道，此通道数据位数为8bit)
    cudaChannelFormatDesc uchar1_channel_desc = cudaCreateChannelDesc(32, 0, 0, 0, cudaChannelFormatKindUnsigned);
    //分配cuda数组
    CHECKCUDA(cudaMallocArray(&cudaArray, &uchar1_channel_desc, cols, rows));
    //创建资源desc
    cudaResourceDesc resource_desc;
    createDefault2DResourceDescriptor(resource_desc, cudaArray); // 初始化资源描述子并将资源数据cudaArray赋值进去
    //分配纹理
    CHECKCUDA(cudaCreateTextureObject(&texture, &resource_desc, &unsigned_texture_desc, 0));
    CHECKCUDA(cudaCreateSurfaceObject(&surface, &resource_desc));
}

void SparseSurfelFusion::createUnsignedTextureSurface(const unsigned int rows, const unsigned int cols, CudaTextureSurface& textureCollect)
{
    createUnsignedTextureSurface(rows, cols, textureCollect.texture, textureCollect.surface, textureCollect.cudaArray);
}
...
cudaTextureObject_t SparseSurfelFusion::create1DLinearTexture(const DeviceArray<float>& array) {
    cudaTextureDesc texture_desc;
    memset(&texture_desc, 0, sizeof(cudaTextureDesc));
    texture_desc.normalizedCoords = 0;
    texture_desc.addressMode[0] = cudaAddressModeBorder; //在边界之外返回0
...
cudaTextureObject_t SparseSurfelFusion::create1DLinearTexture(const DeviceBufferArray<float>& array) {
