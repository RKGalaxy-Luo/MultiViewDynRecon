...
#define UPM_ROUNDNUM(x) ((int)(x + 0.5f))//我们留下的一个额外的边距，防止关键点是在图像的边缘
// An extra margin we leave in case of the keypoint is in the margin of the image

#define UPM_DEGREES_TO_RADS 0.017453292519943295 // (M_PI / 180.0)角度到弧度

#define UPM_BELID_EXTRA_RATIO_MARGIN 1.75//额外比例余量

inline bool isKeypointInTheBorder(const cv::KeyPoint& kp, const cv::Size& imgSize, const cv::Size& patchSize = { 32, 32 }, float scaleFactor = 1) {
    //这是一个正确的测量，但是因为我们将与尺寸一半进行比较，因此用这个作为边框的大小。
...
/**
 * \brief               校正符合描述符的弱学习器的坐标
 * \param wlPatchParams 弱学习器的Patch参数
 * \param wlImageParams 弱学习器的图像参数
 * \param kp            关键点
 * \param scaleFactor   尺度因子
 * \param patchSize     Patch大小
 */
inline void rectifyABWL(const std::vector<BEBLID::ABWLParams>& wlPatchParams,
    std::vector<BEBLID::ABWLParams>& wlImageParams,
...
/**
 * @brief Computes the WL resoponse for a certain weak learner      计算某弱学习器的WL响应
 * @param wlImageParams
 * @param integralImage
 * @return
 */
inline float computeABWLResponse(const BEBLID::ABWLParams& wlImageParams, const cv::Mat& integralImage) {
    ...
    // Read the integral image values for the first box                         读取第一个框的积分图像值
    A = ptr[idx1];
    B = ptr[idx2];
    C = ptr[idx3];
    D = ptr[idx4];

    // Calculate the mean intensity value of the pixels in the box              计算框中像素的平均强度值
    sum1 = A + D - B - C;
    box_area1 = (box1y2 - box1y1) * (box1x2 - box1x1);
    assert(box_area1 > 0);
    average1 = sum1 / box_area1;

    // Calculate the indices on the integral image where the box falls          计算方框所在的积分图像上的索引
...
void BEBLID::computeBEBLID(const cv::Mat& integralImg, const std::vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors) {
    assert(!integralImg.empty());//assert作用是如果它的条件返回错误，则终止程序执行，原型定义
    assert(descriptors.rows == keypoints.size());//如果描述符的维度不等于关键点的个数，终止程序

    const int* integralPtr = integralImg.ptr<int>();
    cv::Size frameSize(integralImg.cols - 1, integralImg.rows - 1);

    // Parallel Loop to process descriptors并行循环到进程描述符
#ifndef UPM_BEBLID_PARALLEL
    const cv::Range range(0, keypoints.size());
#else
    ParallelLambdaWrapper loop_f([&](const cv::Range& range) {
#endif
        // Get a pointer to the first element in the range获取指向范围内第一个元素的指针
        ABWLParams* wl;
        float responseFun;
        ...
        for (kpIdx = range.start; kpIdx < range.end; kpIdx++) {
            // Rectify the weak learners coordinates using the keypoint information 利用关键点信息对弱学习者坐标进行校正
            ...
            if (isKeypointInTheBorder(keypoints[kpIdx], frameSize, patch_size_, scale_factor_)) {
                // Code to process the keypoints in the image margins 代码处理在图像边缘的关键点
                for (wlIdx = 0; wlIdx < wl_params_.size(); wlIdx++) {
                    bit_idx = 7 - wlIdx % 8;
                    responseFun = computeABWLResponse(imgWLParams[wlIdx], integralImg);
                    // Set the bit to 1 if the response function is less or equal to the threshod 如果响应函数小于或等于阈值，则置位为1
                    byte |= (responseFun <= wl_params_[wlIdx].th) << bit_idx;
                    // If we filled the byte, save it 如果我们填满了字节，保存它
                    if (bit_idx == 0) {
                        *d = byte;
                        byte = 0;
                        d++;
                ...
                // Code to process the keypoints in the image center 代码处理在图像中心的关键点
                wl = imgWLParams.data();
                for (wlIdx = 0; wlIdx < wl_params_.size(); wlIdx++) {
                    bit_idx = 7 - wlIdx % 8;

                    // For the first box, we calculate its margin coordinates 对于第一个框，我们计算它的边距坐标
                    ...
                    // For the second box, we calculate its margin coordinates 对于第二个框，我们计算它的边距坐标
                    ...
                    // Check that all the image indices are inside the image. Only in debug mode检查所有的图像索引是否在图像内部。仅在调试模式下
