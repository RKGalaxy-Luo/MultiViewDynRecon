 ...
 *********************************************************************/
#include "DrawMesh.h"
__device__ __forceinline__ void SparseSurfelFusion::device::KnnHeapDevice::update(unsigned int idx, float dist)
{
	if (dist < distance.x) {
		distance.x = dist;
		index.x = idx;
...
__global__ void SparseSurfelFusion::device::CalculateMeshNormalsKernel(DeviceArrayView<Point3D<float>> verticesArray, DeviceArrayView<TriangleIndex> indicesArray, const unsigned int meshCount, Point3D<float>* normalsArray)
{
...
__global__ void SparseSurfelFusion::device::CountConnectedTriangleNumKernel(DeviceArrayView<TriangleIndex> indicesArray, const unsigned int meshCount, unsigned int* ConnectedTriangleNum)
{
	const unsigned int idx = threadIdx.x + blockDim.x * blockIdx.x;
	if (idx >= meshCount) return;
...
__global__ void SparseSurfelFusion::device::VerticesNormalsSumKernel(const Point3D<float>* meshNormals, DeviceArrayView<TriangleIndex> indicesArray, const unsigned int meshCount, Point3D<float>* VerticesNormalsSum)
{
	const unsigned int idx = threadIdx.x + blockDim.x * blockIdx.x;
	if (idx >= meshCount)	return;
...
__global__ void SparseSurfelFusion::device::AdjustModelPositionKernel(DeviceArrayView<Point3D<float>> originVertices, const mat34 AdjustModelSE3, const unsigned int verticesNum, Point3D<float>* adjustedVertices)
{
	const unsigned int idx = threadIdx.x + blockDim.x * blockIdx.x;
	if (idx >= verticesNum)	return;
	float3 pos = make_float3(originVertices[idx].coords[0], originVertices[idx].coords[1], originVertices[idx].coords[2]);
	pos = AdjustModelSE3.rot * pos + AdjustModelSE3.trans;
	adjustedVertices[idx].coords[0] = pos.x;
	adjustedVertices[idx].coords[1] = pos.y;
	adjustedVertices[idx].coords[2] = pos.z;
}
...
void SparseSurfelFusion::DrawMesh::CalculateMeshNormals(DeviceArrayView<Point3D<float>> meshVertices, DeviceArrayView<TriangleIndex> meshTriangleIndices, cudaStream_t stream)
{
	...
	device::AdjustModelPositionKernel << <grid_Vertex, block_Vertex, 0, stream >> > (meshVertices, AdjustModelSE3, VerticesCount, MeshVertices.Ptr());
