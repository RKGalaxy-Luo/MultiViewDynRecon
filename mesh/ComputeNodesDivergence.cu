...
//#endif // CHECK_MESH_BUILD_TIME_COST
	...
	for (int i = left; i < CalculatedNodeNum; i++) {		// [0, CoarserLevelNum]层节点总数
		...
		unsigned int* coverNums = NULL;			// 当前层每个邻居所覆盖的maxDepth层节点数量的累和
		...
		float* divg = NULL;						// 当前Coarser节点下属maxDepth层的节点，所计算出的maxDepth层节点的散度
		...
		//float divgSumHost = 0;
		//CHECKCUDA(cudaMemcpyAsync(&divgSumHost, divgSum, sizeof(float), cudaMemcpyDeviceToHost, stream));
		//CHECKCUDA(cudaStreamSynchronize(stream));
		//printf("depth = %d  index = %d  divgSum = %.5f\n", depth, i, divgSumHost);
