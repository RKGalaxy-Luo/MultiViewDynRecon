...
namespace SparseSurfelFusion {
    ...
    public:
        ...
        __host__ __device__ ~ConfirmedPPolynomial(void) = default;
    ...
    /**
     * \brief 计算带起点的多项式函数在val处的值：f(x) x = val.
     * 
     * \param cp 多项式函数
     * \param val 自变量值
     * \return f(val)
     */
    template<int Degree, int PolyCount> __device__ float value(const ConfirmedPPolynomial<Degree, PolyCount>& cp, const float& val) {
        float res = 0;
