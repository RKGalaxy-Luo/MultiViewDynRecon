...
namespace SparseSurfelFusion {
	namespace device {
		...
		union uint2float_union_device_mesh {
			unsigned i_value;
			float f_value;
		};

		__device__ __forceinline__ void uint_decode_rgb_device_mesh(const unsigned encoded, uchar3& rgb) {
			rgb.x = ((encoded & 0x00ff0000) >> 16);
			rgb.y = ((encoded & 0x0000ff00) >> 8);
			rgb.z = ((encoded & 0x000000ff) /*0*/);
		}
		__device__ __forceinline__ unsigned float_as_uint_device_mesh(float f_value) {
			device::uint2float_union_device_mesh u;
			u.f_value = f_value;
			return u.i_value;
		}
		__device__ __forceinline__ void float_decode_rgb_device_mesh(const float encoded, uchar3& rgb) {
			const unsigned int unsigned_encoded = float_as_uint_device_mesh(encoded);
			uint_decode_rgb_device_mesh(unsigned_encoded, rgb);
		}
...
__global__ void SparseSurfelFusion::device::getCoordinateAndNormalKernel(OrientedPoint3D<float>* point, DeviceArrayView<DepthSurfel> PointCloud, const unsigned int pointsCount)
{
	...
	uchar3 rgb;
	device::float_decode_rgb_device_mesh(PointCloud[idx].ColorAndTime.x, rgb);
	float r = rgb.z / 255.0f;
	float g = rgb.y / 255.0f;
	float b = rgb.x / 255.0f;
	//if (idx % 1000 == 0)	printf("idx = %d   r = %.5f   g = %.5f   b = %.5f\n", idx, r, g, b);
	point[idx].color = Point3D<float>(r, g, b);
...
#pragma unroll	// 展开循环，加速运算
		for (int child = 0; child < 8; child++) {						// 计算Children在NodeArray的位置
			...
			NodeArray[idx].children[child] = child + 1;		// 第一层节点可以有某些节点完全是空的，但是作为第一层节点必须满排，否则会有这里会有index错误导致后续用LUT表的时候，找到错误的邻居
		}
	}
	else {
