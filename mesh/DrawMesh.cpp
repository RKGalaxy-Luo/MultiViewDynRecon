...
SparseSurfelFusion::DrawMesh::DrawMesh()
{
	...
	window = glfwCreateWindow(WindowWidth, WindowHeight, "MeshRender", NULL, NULL);
...
void SparseSurfelFusion::DrawMesh::DrawRenderedMesh(cudaStream_t stream)
{
	...
	//while (true) {
	//	if (CheckPressESC(window)) break;	// 检查是否按下ESC
	//	clearWindow();
...
void SparseSurfelFusion::DrawMesh::drawMesh(glm::mat4& view, glm::mat4& projection, glm::mat4& model)
{
	...
	meshShader.SetUniformVector("lightPos", 0.0f, -.0f, 2.0f);
	...
	//view = glm::lookAt(glm::vec3(0.0f, 1.0f, -2.5f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));

	meshShader.setUniformMat4(std::string("view"), view);
	//meshShader.SetUniformVector("viewPos", glm::vec3(camX, 2.0f, camZ));
	meshShader.SetUniformVector("viewPos", glm::vec3(0.0f, 0.0f, 2.0f));
