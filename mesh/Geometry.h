...
namespace SparseSurfelFusion {
    ...
    public:
        ...
        bool GetTriangleIndices(DeviceBufferArray<TriangleIndex>& triangleIndices, cudaStream_t stream = 0);  // 获得需要绘制的三角形index，直接拷贝到GPU

        bool GetVertexArray(std::vector<float>& vertexArray);                       // 获得顶点数组
        bool GetVertexArray(std::vector<Point3D<float>>& vertexArray);              // 获得顶点数组
        bool GetVertexArray(DeviceBufferArray<Point3D<float>>& vertexArray, cudaStream_t stream = 0);        // 获得顶点数组,直接拷贝到GPU
