...
namespace SparseSurfelFusion {
	namespace device {
		...
		/**
		 * \brief 生成顶点的vertexNums和顶点的vertexAddress的核函数【重建曲面经过边(边在EdgeArray索引为idx)，则verNums[idx] = 1，否则为0】.
		 *
		 * \param EdgeArray 边数组
		 * \param NodeArray 节点数组
		 * \param vvalue 顶点隐式函数值
		 * \param vexNums 辅助计算节点位置VertexAddress
		 * \param markValidVertex 标记有效的相交顶点在Array中的位置(位置与有效相交边在Array中的位置相同)
		 */
		...
		/**
		 * \brief 计算两点之间的插入点，变相计算重建曲面与边的交点.
		 * 
		 * \param p1 边的顶点1
		 * \param p2 边的顶点2
		 * \param dim 需要进行插入的维度
		 * \param v1 顶点1的隐式函数值
		 * \param v2 顶点2的隐式函数值
		 * \param out 【输入】插入点的坐标，边与重建曲面相交点的坐标
		 */
		 ...
		 * \brief 获得有效的三角面元索引.
		 */
		__global__ void markValidMeshTriangleIndex(const TriangleIndex* TriangleBuffer, const unsigned int allTriNums, const unsigned int verticesNum, bool* markValidTriangleIndex);
	}
	...
	public:
		...
		DeviceArrayView<Point3D<float>> GetRebuildMeshVertices() {
			//printf("MeshTriangleVertexCount = %lld\n", MeshTriangleVertex.ArraySize());
		...
		DeviceArrayView<TriangleIndex> GetRebuildMeshTriangleIndices() {
			//printf("MeshTriangleIndexCount = %lld\n", MeshTriangleIndex.ArraySize());
			return MeshTriangleIndex.ArrayView();
		}

	private:
		DeviceBufferArray<float> vvalue;								// 【论文参数】顶点隐式函数值
		DeviceBufferArray<int> vexNums;									// 【论文参数】顶点的数量：重建曲面经过边(边在EdgeArray索引为idx)，则verNums[idx] = 1，否则为0
		DeviceBufferArray<int> vexAddress;								// 【论文参数】顶点的位置：对verNums求前缀和，记录当前是第几个与重建曲面相交的边
		DeviceBufferArray<int> triNums;									// 【论文参数】三角形数量：当前maxDepth层节点对应的cube有多少个三角形
		DeviceBufferArray<int> triAddress;								// 【论文参数】三角形位置：对triNums求前缀和，方便将当前这种cube类型的三角形全部存于TriangleBuffer，主要是一个内存偏移的作用
		DeviceBufferArray<int> cubeCatagory;							// 记录这是属于哪一种类型的立方体，方便使用Marching Cubes算法构建网格

		DeviceBufferArray<bool> markValidVertex;						// 标记有效的相交顶点在Array中的位置(位置与有效相交边在Array中的位置相同)

		DeviceBufferArray<bool> markValidSubdividedNode;				// 标记节点是否可以被细分优化

		DeviceBufferArray<TriangleIndex> MeshTriangleIndex;				// 三角网格绘制索引
		DeviceBufferArray<bool> markValidTriangleIndex;					// 标记有效的网格索引
		DeviceBufferArray<Point3D<float>> MeshTriangleVertex;			// 网格顶点
		DeviceBufferArray<bool> markValidTriangleVertex;				// 标记有效的网格顶点

		DeviceBufferArray<OctNode> SubdivideNode;						// 细分节点，将非maxDepth层的节点中：①没有孩子节点的节点，②该节点的cube存在"三角形"或者是"面边相交"

		int SubdivideDepthCount[Constants::maxDepth_Host] = { 0 };		// 【HostArray】细分节点每一层节点的数量
		int SubdivideDepthAddress[Constants::maxDepth_Host] = { 0 };	// 【HostArray】细分节点每层在SubdivideNode的偏移

		int SubdivideNodeNumHost = 0;									// 细分节点的个数
		SynchronizeArray<int> SubdivideDepthBuffer;						// 记录SubdivideNode数组细分节点的深度

		const unsigned int finerDepth = 5;								// 细化节点的层：[0, finerDepth)是Coarser层, [finerDepth, maxDepth]是Finer层
		 ...
		 * \param allTriNums 所有有效三角形数量
		 */
		void insertTriangle(const Point3D<float>* VertexBuffer, const int allVexNums, const TriangleIndex* TriangleBuffer, const int allTriNums, cudaStream_t stream);
