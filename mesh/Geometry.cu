...
bool SparseSurfelFusion::CoredVectorMeshData::GetTriangleIndices(DeviceBufferArray<TriangleIndex>& triangleIndices, cudaStream_t stream)
{
    if (triangles.size() > 0) {
        printf("Here2!\n");
        triangleIndices.ResizeArrayOrException(triangles.size());
        CHECKCUDA(cudaMemcpyAsync(triangleIndices.Ptr(), triangles.data(), sizeof(TriangleIndex) * triangles.size(), cudaMemcpyHostToDevice, stream));
        return true;
    }
    return false;
}
...
bool SparseSurfelFusion::CoredVectorMeshData::GetVertexArray(DeviceBufferArray<Point3D<float>>& vertexArray, cudaStream_t stream)
{
    if (inCorePoints.size() > 0) {
        vertexArray.ResizeArrayOrException(inCorePoints.size());
        CHECKCUDA(cudaMemcpyAsync(vertexArray.Ptr(), inCorePoints.data(), sizeof(Point3D<float>) * inCorePoints.size(), cudaMemcpyHostToDevice, stream));
        return true;
    }
    return false;
}
...
SparseSurfelFusion::MeshData::MeshData()
{
