...
namespace SparseSurfelFusion {
	...
	public:
		...
		/**
		 * \brief 获得dx：每个节点(0 - maxDepth层)的指示函数(隐式函数)的值.
		 * 
		 * \return 
		 */
	...
	private:
		...
		DeviceBufferArray<float> dx;// 散度的Laplace迭代后的解，表示每个节点(0 - maxDepth层)的指示函数(隐式函数)的值

		DeviceBufferArray<float> DensePointsImplicitFunctionValue;	// 稠密点隐函数值

		float isoValue = 0.0f;		// 【根据maxDepth层节点计算】重建曲面的等值，当一个点隐函数值大于isoValue，那么则这个点在曲面外面；当一个点隐函数值小于isoValue则点在曲面内部
	};
}
