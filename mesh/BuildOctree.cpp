...
void SparseSurfelFusion::BuildOctree::BuildNodesArray(DeviceArrayView<DepthSurfel> depthSurfel, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::PointCloud<pcl::Normal>::Ptr normals, cudaStream_t stream)
{
	...
	//// 包围盒可视化
	//BoundBoxVisualization(cloud, sampleOrientedPoints.ArrayView(), MaxPoint, MinPoint);
	...
	//// 可视化检查Octree划分区域
	//CheckOctreeDividedArea(cloud, sampleOrientedPoints.ArrayView());
...
void SparseSurfelFusion::BuildOctree::BoundBoxVisualization(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, DeviceArrayView<OrientedPoint3D<float>> points, Point3D<float> MaxPoint, Point3D<float> MinPoint)
{
	//----------------Host循环加点测试--------------
	std::vector<OrientedPoint3D<float>> pointsHost;
	points.Download(pointsHost);
	for (int i = 0; i < pointsHost.size(); i++) {
		pcl::PointXYZ point;
		point.x = pointsHost[i].point.coords[0];
		point.y = pointsHost[i].point.coords[1];
		point.z = pointsHost[i].point.coords[2];
...
void SparseSurfelFusion::BuildOctree::CheckOctreeDividedArea(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, DeviceArrayView<OrientedPoint3D<float>> points)
{
	std::vector<OrientedPoint3D<float>> pointsHost;
	points.Download(pointsHost);
	for (int i = 0; i < pointsHost.size(); i++) {
	...
	viewer->addCoordinateSystem(0.1);
	viewer->addPointCloud<pcl::PointXYZ>(cloud, single_color, "sample cloud");
	//viewer->addCube(0.5f, 1.0f, 0, 0.5f, 0.5f, 1.0f, 1.0f, 0, 0, "cube");	// idx = 5
	while (!viewer->wasStopped())
	{
		viewer->spinOnce(100);
	}
