...
namespace SparseSurfelFusion {
	...
	private:
		SurfelGeometry::Ptr m_surfel_geometry[MAX_CAMERA_COUNT][2];	// surfelGeometry的读写接口，与外部surfelGeometry完全同步
		...
		const unsigned int clipedImageSize = CLIP_HEIGHT * CLIP_WIDTH;
	public:
		using Ptr = std::shared_ptr<GeometryReinitProcessor>;
		GeometryReinitProcessor(SurfelGeometry::Ptr surfel_geometry[MAX_CAMERA_COUNT][2], Intrinsic *intrinsicArray); //Init with double buffer
		~GeometryReinitProcessor();
		NO_COPY_ASSIGN_MOVE(GeometryReinitProcessor);
		...
		void ProcessReinitObservedOnlySerial(unsigned& num_remaining_surfel, unsigned& num_appended_surfel, unsigned* number, const unsigned int frameIdx, cudaStream_t stream = 0);
	...
	public:
		 ...
		 */
	private:
		PrefixSum remainingIndicatorPrefixsum;
		PrefixSum appendedIndicatorPrefixsum;
	...
	public:
		void CompactSurfelToAnotherBufferSync(unsigned int& num_remaining_surfels, unsigned int& num_appended_surfels, unsigned int* number, cudaStream_t stream = 0);
	};
