...
#include <base/safe_call_utils.h>
#include "BlockPCG.h"
#include "BinBlockCSR.h"
...
template<int BlockDim>
SparseSurfelFusion::BlockPCG<BlockDim>::BlockPCG(size_t max_matrix_size, cudaStream_t stream) {
	//Allocate the buffer
	m_max_matrix_size = 0;
...
template<int BlockDim>
void SparseSurfelFusion::BlockPCG<BlockDim>::UpdateCudaStream(cudaStream_t stream) {
	m_stream = stream;
	cublasSafeCall(cublasSetStream(m_cublas_handle, m_stream));
}

template<int BlockDim>
SparseSurfelFusion::BlockPCG<BlockDim>::~BlockPCG() {
	releaseBuffer();
	cublasDestroy(m_cublas_handle);
}

template<int BlockDim>
void SparseSurfelFusion::BlockPCG<BlockDim>::allocateBuffer(size_t max_matrix_size) {
	...
	CHECKCUDA(cudaMalloc((void**)(&delta_old_), sizeof(float)));
	CHECKCUDA(cudaMalloc((void**)(&delta_new_), sizeof(float)));
	CHECKCUDA(cudaMalloc((void**)(&dot_dq_), sizeof(float)));

	//Allocate the page-locked memory for converge checking
	CHECKCUDA(cudaMallocHost((void**)(&delta_0_pagelock_), sizeof(float)));
	CHECKCUDA(cudaMallocHost((void**)(&delta_pagelock_), sizeof(float)));
...
template<int BlockDim>
void SparseSurfelFusion::BlockPCG<BlockDim>::releaseBuffer() {
	...
	CHECKCUDA(cudaFree(delta_old_));
	CHECKCUDA(cudaFree(delta_new_));
	CHECKCUDA(cudaFree(dot_dq_));
	CHECKCUDA(cudaFreeHost(delta_pagelock_));
	CHECKCUDA(cudaFreeHost(delta_0_pagelock_));
...
bool SparseSurfelFusion::BlockPCG<BlockDim>::SetSolverInput(
	...
	FUNCTION_CHECK(actual_size % BlockDim == 0);
	FUNCTION_CHECK_EQ(inv_diag_blks.Size(), BlockDim * BlockDim * num_blks);
	FUNCTION_CHECK_EQ(b.Size(), BlockDim * num_blks);
	FUNCTION_CHECK_EQ(x_init.Size(), BlockDim * num_blks);
...
template<int BlockDim>
SparseSurfelFusion::DeviceArrayView<float> SparseSurfelFusion::BlockPCG<BlockDim>::Solve(size_t max_iterations, bool zero_init) {
	return SolveNoConvergeCheck(max_iterations, zero_init);
}

template<int BlockDim>
SparseSurfelFusion::DeviceArrayView<float> SparseSurfelFusion::BlockPCG<BlockDim>::SolveNoConvergeCheck(size_t max_iteration, bool zero_init) {
	//Do initialize
	if (zero_init) {
		ZeroIntialize();
	}
...
template<int BlockDim>
SparseSurfelFusion::DeviceArrayView<float> SparseSurfelFusion::BlockPCG<BlockDim>::SolveConvergeChecked(
	...
	CHECKCUDA(cudaMemcpyAsync(delta_0_pagelock_, delta_new_, sizeof(float), cudaMemcpyDeviceToHost, m_stream));
	...
	for (auto i = 0; i < max_iteration; i++) {
		...
		//Check converge
		CHECKCUDA(cudaMemcpyAsync(delta_pagelock_, delta_new_, sizeof(float), cudaMemcpyDeviceToHost, m_stream));
...
template<int BlockDim>
void SparseSurfelFusion::BlockPCG<BlockDim>::Initialize(const DeviceArrayView<float>& x_init) {
...
template<int BlockDim>
void SparseSurfelFusion::BlockPCG<BlockDim>::ZeroIntialize() {
	const auto num_blocks = m_actual_matrix_size / BlockDim;
	dim3 init_blk(64);
	dim3 init_grid(divUp(num_blocks, init_blk.x));
...
template<int BlockDim>
void SparseSurfelFusion::BlockPCG<BlockDim>::PerformSparseMV() {
	//Using the handler
...
template<int BlockDim>
void SparseSurfelFusion::BlockPCG<BlockDim>::TexturedSparseMV() {
	//Use the handler
...
template<int BlockDim>
void SparseSurfelFusion::BlockPCG<BlockDim>::PerformSecondUpdate() {
	dim3 update_blk(128);
...
template<int BlockDim>
void SparseSurfelFusion::BlockPCG<BlockDim>::PerformThirdUpdate() {
	dim3 update_blk(128);
	dim3 update_grid(divUp(m_actual_matrix_size, update_blk.x));
