...
void SparseSurfelFusion::JtJMaterializer::AssembleBinBlockCSR(SparseSurfelFusion::DeviceArrayView<float> diagonal_blks, cudaStream_t stream) {
	//Zero out the matrix
	CHECKCUDA(cudaMemsetAsync(m_binblock_csr_data.Ptr(), 0, sizeof(float) * m_binblock_csr_data.Capacity(), stream));

	//The size of the matrix
	FUNCTION_CHECK(diagonal_blks.Size() % 6 == 0);
