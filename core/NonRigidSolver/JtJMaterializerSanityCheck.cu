...
void SparseSurfelFusion::JtJMaterializer::TestSparseMV(
	DeviceArrayView<float> x,
	DeviceArrayView<float> jtj_x_result
) {
	LOGGING(INFO) << "Check materialized spmv given input groundtruth";
	...
	spmv_handler.ApplySpMV(x, DeviceArrayHandle<float>(jtj_x));
	...
	LOGGING(INFO) << "Check done, the relative error between materalized method and matrix free method is " << relative_err;
}