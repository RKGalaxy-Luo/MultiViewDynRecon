...
#pragma unroll
			...
			for (auto iter = threadIdx.x; iter < padded_term_size; iter += thread_blk_size) {
				...
				if (iter < term_size) {
					...
					{
						float term_jt_residual[6] = { 0 };
						computePoint2PointJtResidual(term2jacobian.cross_corr_term, node_idx, typed_term_idx, term_jt_residual);
						fillScalarJtResidualToSharedBlock(term_jt_residual, shared_blks, constants.CrossViewCorrSquared());
					}
					break;
					default:
						term_valid = false;
...
void SparseSurfelFusion::PreconditionerRhsBuilder::ComputeJtResidualIndexed(cudaStream_t stream)
{
...
void SparseSurfelFusion::PreconditionerRhsBuilder::ComputeJtResidualGlobalIteration(cudaStream_t stream) {
	const auto num_nodes = m_node2term_map.offset.Size() - 1;
	m_jt_residual.ResizeArrayOrException(num_nodes * device::jt_dot_blk_size);
	dim3 blk(device::thread_blk_size);
	dim3 grid(num_nodes);
...
void SparseSurfelFusion::PreconditionerRhsBuilder::ComputeJtResidualLocalIteration(cudaStream_t stream) {
