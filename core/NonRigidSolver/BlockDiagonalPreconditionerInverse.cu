...
#include "BlockDiagonalPreconditionerInverse.h"
namespace SparseSurfelFusion {
	namespace device {
		/*template<int BlockDim, int NumThreads = 64>*/
		template<int BlockDim, int NumThreads = 64>
		__global__ void blockDiagonalInverseKernel(const float* A, float* A_inversed, const unsigned num_matrix)
		{
...
void SparseSurfelFusion::BlockDiagonalPreconditionerInverse<BlockDim>::PerformDiagonalInverse(cudaStream_t stream)
{
...
//写一个函数调用surfelwarp::BlockDiagonalPreconditionerInverse<BlockDim> 将其实例化，从而在 obj中找到
void InstantiateTheFunction() {
	SparseSurfelFusion::BlockDiagonalPreconditionerInverse<6>::Ptr initPtr;
	initPtr->PerformDiagonalInverse(0);
}