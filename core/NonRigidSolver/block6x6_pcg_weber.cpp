...
#if defined(_WIN32)
const std::string pcg_data_path = "D:/SparseSurfelFusion/data";
#else
const std::string pcg_data_path = "D:/SparseSurfelFusion/data";
#endif

void SparseSurfelFusion::loadCheckData(
	...
	BinaryFileStream input_fstream(pcg_test_path.string().c_str(), BinaryFileStream::FileOperationMode::ReadOnly);
...
void SparseSurfelFusion::loadCheckData(
	...
	BinaryFileStream input_fstream(pcg_result_path.string().c_str(), BinaryFileStream::FileOperationMode::ReadOnly);
	input_fstream.SerializeRead(&x);
	input_fstream.SerializeRead(&inv_diag_blks);
}


void SparseSurfelFusion::check6x6DiagBlocksInverse(
	...
	for (auto i = 0; i < h_diag_inversed.size(); i++) {
		...
		if (std::abs(check_diag_value) > 2e-3) {
			if (std::abs((check_diag_value - h_diag_value) / h_diag_value) > max_relative_err) {
				...
				LOGGING(INFO) << "The host value and checked value are " << h_diag_value << " and " << check_diag_value;
			}
		}
	}
...
void SparseSurfelFusion::block6x6BuildTripletVector(
...
void SparseSurfelFusion::hostEigenSpMV(
	const std::vector<float>& A_data,
	const std::vector<int>& A_rowptr,
	const std::vector<int>& A_colptr,
	const int matrix_size,
...
void SparseSurfelFusion::checkBlock6x6PCGSolver(
	const std::vector<float>& diag_blks,
	const std::vector<float>& A_data,
	const std::vector<int>& A_colptr,
	const std::vector<int>& A_rowptr,
