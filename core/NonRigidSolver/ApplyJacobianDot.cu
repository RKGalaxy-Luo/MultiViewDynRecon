...
//Assign the static
const unsigned SparseSurfelFusion::ApplyJtJHandlerMatrixFree::kMaxNumScalarResidualTerms = 300000;

void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::applyJacobianDot(DeviceArrayView<float> x, cudaStream_t stream)
{
	//Resize the array
...
void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::ApplyJtJSeparate(
	DeviceArrayView<float> x,
	DeviceArrayHandle<float> jtj_dot_x,
	cudaStream_t stream
) {
