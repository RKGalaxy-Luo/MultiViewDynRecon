...
void SparseSurfelFusion::JtJMaterializer::updateScalarCostJtJBlockHost(
	...
	FUNCTION_CHECK_EQ(num_nodepairs * 36, jtj_flatten.size());
		...
		for (auto i = 0; i < 4; i++) {
			...
			for (auto j = 0; j < 4; j++) {
				...
				if (iter != pair2index.end()) {
					...
					LOGGING(FATAL) << "Cannot find the index for node " << node_i << " and " << node_j << " pair!";
				}
			}
...
void SparseSurfelFusion::JtJMaterializer::updateSmoothCostJtJBlockHost(std::vector<float>& jtj_flatten) {
	const auto num_nodepairs = m_nodepair2term_map.nodepair_term_range.Size();
	FUNCTION_CHECK_EQ(num_nodepairs * 36, jtj_flatten.size());
	...
	for (auto term_idx = 0; term_idx < node_graph.size(); term_idx++) {
		...
		if (iter != pair2index.end()) {
			...
			//Kill it
			LOGGING(FATAL) << "The index is not found, something wrong with the index";
		}
...
void SparseSurfelFusion::JtJMaterializer::updateFeatureCostJtJBlockHost(std::vector<float>& jtj_flatten) {
	const auto num_nodepairs = m_nodepair2term_map.nodepair_term_range.Size();
	FUNCTION_CHECK_EQ(num_nodepairs * 36, jtj_flatten.size());
		...
		for (auto i = 0; i < 4; i++) {
			...
			for (auto j = 0; j < 4; j++) {
				...
				if (iter != pair2index.end()) {
					...
					LOGGING(FATAL) << "The node pair " << node_i << " and " << node_j << " is not found";
				}
			}
		}
	}
...
void SparseSurfelFusion::JtJMaterializer::nonDiagonalBlocksSanityCheck() {
	LOGGING(INFO) << "Sanity check of materialized non-diagonal JtJ block";
	...
	FUNCTION_CHECK_EQ(jtj_blocks.size(), jtj_blocks_dev.size());
	...
	LOGGING(INFO) << "The relative error for non-diagonal jtj blocks is " << relative_err;
}
