...
namespace SparseSurfelFusion {
	...
	/**
	 * \brief 某标量代价对节点SE(3)扭转参数的梯度为INCREMENT，而节点SE(3)本身用DualQuaternion参数化.
	 */
	...
	/**
	 * \brief Term2Jacobian结构，顾名思义，提供了足够的信息来计算从给定项索引到该项所涉及的所有节点的代价梯度。
	 *		  请注意，每个term可能有一个或多个标量成本。在多个标量代价的情况下，雅可比矩阵不能合并。
	 *		  term2jacobian应该在GPU上实现，但可以为调试检查提供主机实现.
	 */
	struct ScalarCostTerm2Jacobian {
		...
		__host__ __forceinline__ void check_size() const {
			...
			FUNCTION_CHECK_EQ(knn_array.Size(), residual_array.Size());
	...
	struct NodeAlignmentError {
		DeviceArrayView<float> nodeAccumlatedError;
