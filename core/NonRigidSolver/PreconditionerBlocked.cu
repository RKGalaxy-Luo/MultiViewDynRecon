...
#include "geometry_term2jacobian.h"
...
#pragma unroll
			...
			for (auto iter = threadIdx.x; iter < padded_term_size; iter += warp_size) {
				...
				if (iter < term_size) {
					...
					{
						float channelled_jacobian[18] = { 0 };
						computeJtJDiagonalJacobian(term2jacobian.cross_corr_term, node_idx, typed_term_idx, channelled_jacobian);
						fillChannelledJtJToSharedBlock(channelled_jacobian, shared_blks, constants.CrossViewCorrSquared());
					}
					break;
...
#pragma unroll
			...
			for (auto iter = threadIdx.x; iter < padded_term_size; iter += warp_size) {
				...
				if (__all_sync(0xFFFFFF, !term_valid))
					continue;
...
void SparseSurfelFusion::PreconditionerRhsBuilder::ComputeDiagonalBlocks(cudaStream_t stream) {
...
void SparseSurfelFusion::PreconditionerRhsBuilder::ComputeDiagonalPreconditionerGlobalIteration(cudaStream_t stream) {
	//Check the constants
	FUNCTION_CHECK(m_penalty_constants.Density() < 1e-7f);
...
void SparseSurfelFusion::PreconditionerRhsBuilder::ComputeDiagonalPreconditionerInverse(cudaStream_t stream) {
