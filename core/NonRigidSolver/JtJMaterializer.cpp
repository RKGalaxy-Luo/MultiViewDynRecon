...
SparseSurfelFusion::JtJMaterializer::JtJMaterializer() {
...
void SparseSurfelFusion::JtJMaterializer::AllocateBuffer() {
	m_nondiag_blks.AllocateBuffer(36 * Constants::kMaxNumNodePairs);
	m_binblock_csr_data.AllocateBuffer(36 * (Constants::kMaxNumNodePairs + Constants::maxNodesNum));
	m_spmv_handler = std::make_shared<ApplySpMVBinBlockCSR<6>>();
}

void SparseSurfelFusion::JtJMaterializer::ReleaseBuffer() {
	m_nondiag_blks.ReleaseBuffer();
}

void SparseSurfelFusion::JtJMaterializer::SetInputs(
...
void SparseSurfelFusion::JtJMaterializer::BuildMaterializedJtJNondiagonalBlocks(cudaStream_t stream) {
