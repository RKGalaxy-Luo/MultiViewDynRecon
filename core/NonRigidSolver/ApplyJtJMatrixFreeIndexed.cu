...
void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::ApplyJtJIndexed(DeviceArrayView<float> x, DeviceArrayHandle<float> jtj_dot_x, cudaStream_t stream)
{
	//simple sanity check
	FUNCTION_CHECK_EQ(x.Size(), jtj_dot_x.Size());
	FUNCTION_CHECK(x.Size() % 6 == 0);
