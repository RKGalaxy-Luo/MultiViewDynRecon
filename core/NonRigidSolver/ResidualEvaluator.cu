...
#if defined(__CUDACC__)		//»Áπ˚”…NVCC±‡“Î∆˜±‡“Î
#include <cub/cub.cuh>
#endif
...
namespace SparseSurfelFusion {
	namespace device {
		...
		__device__ __forceinline__ void computeCrossCorrResidual(
			const Point2PointICPTerm2Jacobian& term2jacobian,
			unsigned typed_term_idx,
			float residual[3]
		) {
		...
		__global__ void computeTermResidualKernel(
			...
			case TermType::CrossViewCorr: {
				float cross_residual[3];
				computeCrossCorrResidual(term2jacobian.cross_corr_term, typed_term_idx, cross_residual);
				for (auto i = 0; i < 3; i++) {
					auto value = constants.CrossViewCorr() * cross_residual[i];
					if (use_square) value = value * value;
					residuals[scalar_term_idx + i] = value;
				}
			}
...
void SparseSurfelFusion::ResidualEvaluator::AllocateBuffer() {
	const unsigned int num_pixels = imageWidth * imageHeight;
	...
	const unsigned int max_cross_corr_term = Constants::MaxCrossViewMatchedPairs;

	//The residual by terms
	const auto scalar_term_size = max_dense_depth_terms + max_density_terms + max_foreground_terms + 3 * (max_node_graph_terms + max_feature_terms + max_cross_corr_term);
	m_termwise_residual.AllocateBuffer(scalar_term_size);

	//The residual by nodes
	m_nodewise_residual.AllocateBuffer(Constants::maxNodesNum);
	...
	CHECKCUDA(cudaMallocHost((void**)(&m_residual_value_pagelock), sizeof(float)));
}

void SparseSurfelFusion::ResidualEvaluator::ReleaseBuffer() {
	...
	CHECKCUDA(cudaFreeHost(m_residual_value_pagelock));
}


void SparseSurfelFusion::ResidualEvaluator::ComputeResidualByTerms(cudaStream_t stream) {
...
void SparseSurfelFusion::ResidualEvaluator::CollectResidualByNodes(cudaStream_t stream) {

}

void SparseSurfelFusion::ResidualEvaluator::CollectTotalResidual(cudaStream_t stream) {
...
float SparseSurfelFusion::ResidualEvaluator::SyncQueryTotalResidualHost(cudaStream_t stream) {
	const float* dev_ptr = m_residual_prefixsum.Ptr() + m_residual_prefixsum.ArraySize() - 1;
	CHECKCUDA(cudaMemcpyAsync(m_residual_value_pagelock, dev_ptr, sizeof(float), cudaMemcpyDeviceToHost, stream));
