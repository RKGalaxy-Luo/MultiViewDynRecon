...
void SparseSurfelFusion::PreconditionerRhsBuilder::updateScalarCostJtJDiagonalHost(
	...
	FUNCTION_CHECK_EQ(jtj_flatten.size(), num_nodes * 36);
	...
	//Simple sanity check
	FUNCTION_CHECK_EQ(knn_array.size(), knn_weight_array.size());
	FUNCTION_CHECK_EQ(knn_array.size(), twist_gradient_array.size());
...
void SparseSurfelFusion::PreconditionerRhsBuilder::updateSmoothJtJDiagonalHost(std::vector<float>& jtj_flatten) {
...
void SparseSurfelFusion::PreconditionerRhsBuilder::updateFeatureJtJDiagonalHost(std::vector<float>& jtj_flatten) {
	//Download the required data
	...
	//Simple check
	FUNCTION_CHECK_EQ(knn_arr.size(), depth_vertex_arr.size());
	FUNCTION_CHECK_EQ(knn_arr.size(), reference_vertex_arr.size());
	FUNCTION_CHECK_EQ(knn_arr.size(), knn_weight_arr.size());
...
void SparseSurfelFusion::PreconditionerRhsBuilder::updateCrossViewJtJDiagonalHost(std::vector<float>& jtj_flatten)
{
	//Download the required data
	auto term2jacobian_dev = m_term2jacobian_map.cross_corr_term;
	std::vector<float4> depth_vertex_arr, reference_vertex_arr, knn_weight_arr;
	std::vector<ushort4> knn_arr;
	std::vector<DualQuaternion> node_se3_arr;
	term2jacobian_dev.target_vertex.Download(depth_vertex_arr);
...
void SparseSurfelFusion::PreconditionerRhsBuilder::diagonalPreconditionerSanityCheck() {
	LOGGING(INFO) << "Check the diagonal elements of JtJ";
	...
	FUNCTION_CHECK_EQ(diagonal_blks_dev.size(), 36 * (m_node2term_map.offset.Size() - 1));
	...
	updateCrossViewJtJDiagonalHost(jtj_diagonal);
	//Check it
	auto relative_err = maxRelativeError(diagonal_blks_dev, jtj_diagonal, 0.01f);

	LOGGING(INFO) << "The relative error is " << relative_err;
}
