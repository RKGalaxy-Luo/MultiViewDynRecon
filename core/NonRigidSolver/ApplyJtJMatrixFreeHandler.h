...
namespace SparseSurfelFusion {
	...
	public:
		...
		DEFAULT_CONSTRUCT_DESTRUCT(ApplyJtJHandlerMatrixFree);
		NO_COPY_ASSIGN(ApplyJtJHandlerMatrixFree);
		...
		void ApplyJtJ(DeviceArrayView<float> x, DeviceArrayHandle<float> jtj_dot_x, cudaStream_t stream = 0);

		//The inherted interface
		size_t MatrixSize() const override { return 6 * (m_node2term_map.offset.Size() - 1); }
		void ApplySpMV(DeviceArrayView<float> x, DeviceArrayHandle<float> spmv_x, cudaStream_t stream) override;
		 ...
		 */
	public:
		void ApplyJtJIndexed(DeviceArrayView<float> x, DeviceArrayHandle<float> jtj_dot_x, cudaStream_t stream = 0);
		void ApplyJtJAtomic(DeviceArrayView<float> x, DeviceArrayHandle<float> jtj_dot_x, cudaStream_t stream = 0);
	...
	private:
		...
		void applyJacobianTranposeDot(DeviceArrayHandle<float> jtj_dot_x, cudaStream_t stream);
	public:
		void ApplyJtJSeparate(DeviceArrayView<float> x, DeviceArrayHandle<float> jtj_dot_x, cudaStream_t stream = 0);
