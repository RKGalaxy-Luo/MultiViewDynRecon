...
#include <core/AlgorithmTypes.cuh>
...
void SparseSurfelFusion::checkPrefixSum() {
	//The size of testing
...
void SparseSurfelFusion::checkKeyValueSort() {
...
void SparseSurfelFusion::checkFlagSelection() {
	std::vector<int> test_sizes;
	test_sizes.push_back(1000000); test_sizes.push_back(3000000);
	//The selector
	...
	for (auto i = 0; i < test_sizes.size(); i++) {
		...
		flag_selector.validSelectedIndex.download(selected_idx_host);
...
void SparseSurfelFusion::checkUniqueSelection() {
	std::vector<int> test_sizes;
	test_sizes.push_back(1000000); test_sizes.push_back(3000000);
	//The selector
