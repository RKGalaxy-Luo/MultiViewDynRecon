 ...
 */
void SparseSurfelFusion::PreconditionerRhsBuilder::ComputeJtResidual(cudaStream_t stream) {
 ...
 */
void SparseSurfelFusion::PreconditionerRhsBuilder::updateScalarCostJtResidualHost(
	...
	FUNCTION_CHECK_EQ(num_nodes * 6, jt_residual.size());
...
void SparseSurfelFusion::PreconditionerRhsBuilder::updateSmoothJtResidualHost(std::vector<float>& jt_residual)
{
...
void SparseSurfelFusion::PreconditionerRhsBuilder::updateFeatureJtResidualHost(std::vector<float>& jt_residual) {
...
void SparseSurfelFusion::PreconditionerRhsBuilder::updateCrossViewJtResidualHost(std::vector<float>& jt_residual)
{
	const auto term2jacobian = m_term2jacobian_map.cross_corr_term;
	std::vector<float4> ref_vertex_arr, depth_vertex_arr, knn_weight_arr;
	std::vector<ushort4> knn_arr;
	std::vector<DualQuaternion> node_se3;
	term2jacobian.node_se3.Download(node_se3);
...
void SparseSurfelFusion::PreconditionerRhsBuilder::jacobianTransposeResidualSanityCheck() {
	LOGGING(INFO) << "Check the elements of Jt Residual";
