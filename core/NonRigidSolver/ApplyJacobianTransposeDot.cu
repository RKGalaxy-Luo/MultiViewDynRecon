...
void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::applyJacobianTranposeDot(DeviceArrayHandle<float> jtj_dot_x, cudaStream_t stream)
{
	const auto num_nodes = m_node2term_map.offset.Size() - 1;
	FUNCTION_CHECK_EQ(jtj_dot_x.Size(), device::jt_dot_blk_size * num_nodes);
