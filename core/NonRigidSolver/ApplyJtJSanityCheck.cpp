...
void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::applyJtJSanityCheck(DeviceArrayView<float> x, DeviceArrayView<float> jtj_dot_x)
{
	...
	FUNCTION_CHECK_EQ(x.Size(), desired_size);
	FUNCTION_CHECK_EQ(jtj_dot_x.Size(), desired_size);
	...
	LOGGING(INFO) << "The relative error of applying JtJ is " << max_relative_err << " for element value " << vec_0[max_err_idx] << " and " << vec_1[max_err_idx];
}


void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::testHostJtJ() {
	LOGGING(INFO) << "Test of Apply JtJ to a vector using Eigen and Direct";
	...
	LOGGING(INFO) << "The relative error of applying JtJ to random x " << relative_err;
}