...
#pragma unroll
		...
		__global__ void computeJtResidualDirectKernel(
			...
			case TermType::CrossViewCorr:
				atomicApplyJtResidual(term2jacobian.cross_corr_term, typed_term_idx, jt_residual, constants.CrossViewCorrSquared());
				break;
			default:
				break;
			}
...
void SparseSurfelFusion::PreconditionerRhsBuilder::ComputeJtResidualAtomic(cudaStream_t stream) {
	...
	CHECKCUDA(cudaMemsetAsync(m_jt_residual.Ptr(), 0, m_jt_residual.ArraySize() * sizeof(float), stream));
