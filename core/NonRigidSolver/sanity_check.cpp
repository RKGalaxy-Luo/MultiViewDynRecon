...
void SparseSurfelFusion::fillRandomVector(std::vector<float>& vec) {
...
void SparseSurfelFusion::fillRandomVector(std::vector<unsigned int>& vec) {
	std::default_random_engine generator((unsigned int)time(NULL));
...
void SparseSurfelFusion::test_encoding(const size_t test_size) {
	std::default_random_engine generator((unsigned int)time(NULL));
...
void SparseSurfelFusion::fillZeroVector(std::vector<float>& vec) {
	for (auto i = 0; i < vec.size(); i++) {
		vec[i] = 0.0f;
	}
}

double SparseSurfelFusion::maxRelativeError(
	...
	for (auto j = 0; j < std::min(vec_0.size(), vec_1.size()); j++) {
		...
		if (err > small_cutoff) {
			...
			if (relative_err > 1e-3) {
				if (log_output) LOGGING(INFO) << "The relative error for " << j << " element is " << relative_err << " between " << vec_0[j] << " and " << vec_1[j];
			}
		}
...
double SparseSurfelFusion::maxRelativeError(const DeviceArray<float>& vec_0, const DeviceArray<float>& vec_1, const float small_cutoff)
{
...
void SparseSurfelFusion::randomShuffle(std::vector<unsigned int>& key, std::vector<unsigned int>& value) {
...
void SparseSurfelFusion::randomShuffle(std::vector<unsigned>& vec)
{
	std::srand(time(nullptr));
...
void SparseSurfelFusion::fillMultiKeyValuePairs(
...
void SparseSurfelFusion::fillRandomLatticeKey(std::vector<LatticeCoordKey<5>>& lattice, const unsigned num_unique_elements, const unsigned duplicate)
{
	//The random number generator
	std::default_random_engine generator((unsigned int)time(NULL));
	std::uniform_int_distribution<int> distribution;
...
bool SparseSurfelFusion::isElementUnique(const std::vector<unsigned>& vec, const unsigned empty)
{
...
unsigned SparseSurfelFusion::numUniqueElement(const std::vector<unsigned>& vec, const unsigned empty)
{
	//First do a sorting
	std::vector<unsigned> sorted_vec(vec);
	std::sort(sorted_vec.begin(), sorted_vec.end());
...
unsigned SparseSurfelFusion::numUniqueElement(const SparseSurfelFusion::DeviceArray<unsigned int>& vec, const unsigned empty) {
	std::vector<unsigned> h_vec;
	vec.download(h_vec);
	return numUniqueElement(h_vec, empty);
}

unsigned SparseSurfelFusion::numNonZeroElement(const std::vector<unsigned>& vec) {
...
unsigned SparseSurfelFusion::numNonZeroElement(const DeviceArray<unsigned int>& vec) {
	std::vector<unsigned> h_vec;
	vec.download(h_vec);
	return numNonZeroElement(h_vec);
}

unsigned SparseSurfelFusion::numNonZeroElement(const SparseSurfelFusion::DeviceArrayView<unsigned int>& vec) {
	std::vector<unsigned> h_vec;
	vec.Download(h_vec);
	return numNonZeroElement(h_vec);
}

bool SparseSurfelFusion::containsNaN(const std::vector<float4>& vec) {
	for (auto i = 0; i < vec.size(); i++) {
...
bool SparseSurfelFusion::containsNaN(const SparseSurfelFusion::DeviceArrayView<float4>& vec) {
	std::vector<float4> h_vec;
	vec.Download(h_vec);
	return containsNaN(h_vec);
}

void SparseSurfelFusion::applyRandomSE3ToWarpField(
...
void SparseSurfelFusion::applyRandomSE3ToWarpField(
	DeviceArrayHandle<DualQuaternion> node_se3,
	float max_rot, float max_trans
) {
	std::vector<DualQuaternion> h_node_se3;
	node_se3.SyncDeviceToHost(h_node_se3);
	applyRandomSE3ToWarpField(h_node_se3, max_rot, max_trans);
	node_se3.SyncHostToDevice(h_node_se3);
}


double SparseSurfelFusion::differenceL2(const std::vector<float>& vec_0, const std::vector<float>& vec_1) {
...
double SparseSurfelFusion::differenceL2(const DeviceArray<float>& vec_0, const DeviceArray<float>& vec_1) {
	std::vector<float> h_vec_0, h_vec_1;
	vec_0.download(h_vec_0);
	vec_1.download(h_vec_1);
...
void SparseSurfelFusion::residualVectorStatistic(const std::vector<float>& residual_in, int topk, std::ostream& output) {
