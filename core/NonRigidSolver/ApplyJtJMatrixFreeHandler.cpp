...
void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::AllocateBuffer() {
	m_jacobian_dot_x.AllocateBuffer(kMaxNumScalarResidualTerms);
}

void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::ReleaseBuffer() {
	m_jacobian_dot_x.ReleaseBuffer();
}


void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::SetInputs(
...
void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::ApplyJtJ(DeviceArrayView<float> x, DeviceArrayHandle<float> jtj_dot_x, cudaStream_t stream)
{
...
void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::ApplySpMV(DeviceArrayView<float> x, DeviceArrayHandle<float> spmv_x, cudaStream_t stream) {
	ApplyJtJ(x, spmv_x, stream);
}

/* These are method for sanity check
 */
void SparseSurfelFusion::ApplyJtJHandlerMatrixFree::TestApplyJtJ()
{
	...
	DeviceArrayHandle<float> jtj_x_slice(jtj_x_dev.ptr(), jtj_x_dev.size());
