...
namespace SparseSurfelFusion {
	namespace device {
		...
		__global__ void block6x6InitKernel(
			const PtrSize<const float> b,
			const PtrSize<const float> inv_diag_blks,
			PtrSize<float> r,
			PtrSize<float> s,
			PtrSize<float> x
		) {
		...
		__global__ void block6x6PCGKernel_0(
			const PtrSize<const float> A_data,
			const PtrSize<const int> A_colptr,
			const PtrSize<const int> A_rowptr,
			const PtrSize<const float> s,
			PtrSize<float> q
		) {
			const int idx = threadIdx.x + blockIdx.x * blockDim.x;
			if (idx == 0) {
		...
		__global__ void block6x6PCGKernel_1(
			const PtrSize<const float> s,
			const PtrSize<const float> r,
			const PtrSize<const float> q,
			const PtrSize<const float> inv_diag_blks,
			PtrSize<float> x,
			PtrSize<float> t,
			PtrSize<float> p
...
void SparseSurfelFusion::block6x6_pcg_weber(
...
void SparseSurfelFusion::block6x6_diag_inverse(const float* A, float* A_inversed, int num_matrix, cudaStream_t stream)
{
...
void SparseSurfelFusion::block6x6_init_kernel(
	const DeviceArray<float>& b,
...
void SparseSurfelFusion::block6x6_pcg_kernel_0(
	const DeviceArray<float>& A_data,
	const DeviceArray<int>& A_colptr,
	const DeviceArray<int>& A_rowptr,
 ...
 * t <- r - alpha * q; p <- M_inv*t; nu_new <- dot(t, p) */
void SparseSurfelFusion::block6x6_pcg_kernel_1(
	const DeviceArray<float>& s,
...
void SparseSurfelFusion::block6x6_pcg_kernel_2(
 ...
 */
void SparseSurfelFusion::checkBlock6x6Init(
...
void SparseSurfelFusion::checkBlock6x6Kernel_0(
...
void SparseSurfelFusion::checkBlock6x6Kernel_1(
	const std::vector<float>& s,
...
void SparseSurfelFusion::checkBlock6x6Kernel_2(
