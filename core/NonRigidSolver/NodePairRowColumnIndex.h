...
void SparseSurfelFusion::NodePair2TermsIndex::blockRowOffsetSanityCheck() {
	LOGGING(INFO) << "Sanity check for blocked row offset";
	...
	FUNCTION_CHECK(row_offset.size() == row_offset_gpu.size());
	for (int i = 0; i < row_offset.size(); i++) {
		FUNCTION_CHECK(row_offset[i] == row_offset_gpu[i]);
	}

	LOGGING(INFO) << "Check done";
}

void SparseSurfelFusion::NodePair2TermsIndex::blockRowLengthSanityCheck() {
	LOGGING(INFO) << "Sanity check for blocked row length";

	std::vector<unsigned> row_offset;
	m_blkrow_offset_array.ArrayReadOnly().Download(row_offset);
	FUNCTION_CHECK(row_offset.size() == m_num_nodes + 1);
	...
	FUNCTION_CHECK_EQ(row_length.size(), row_length_gpu.size());
	for (auto i = 0; i < row_length.size(); i++) {
		FUNCTION_CHECK(row_length[i] == row_length_gpu[i]);
	}
...
void SparseSurfelFusion::NodePair2TermsIndex::binLengthNonzerosSanityCheck() {
	LOGGING(INFO) << "Sanity check for bin length and non-zeros";
...
void SparseSurfelFusion::NodePair2TermsIndex::binBlockCSRRowPtrSanityCheck() {
	LOGGING(INFO) << "Check of the rowptr for bin-blocked csr";
...
void SparseSurfelFusion::NodePair2TermsIndex::binBlockCSRColumnPtrSanityCheck() {
	LOGGING(INFO) << "Sanity check for the colptr of bin-blocked csr format";
	...
	for (int i = 0; i > JtJ_column_host.size(); i++) {
		FUNCTION_CHECK(JtJ_column_host[i] == JtJ_column_gpu[i]);
	}
