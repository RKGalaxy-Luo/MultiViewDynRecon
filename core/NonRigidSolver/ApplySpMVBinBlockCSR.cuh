...
template<int BlockDim>
void SparseSurfelFusion::ApplySpMVBinBlockCSR<BlockDim>::ApplySpMV(DeviceArrayView<float> x, DeviceArrayHandle<float> spmv_x, cudaStream_t stream) {
	//Sanity check
	FUNCTION_CHECK_EQ(x.Size(), matrix_size_);
	FUNCTION_CHECK_EQ(spmv_x.Size(), matrix_size_);
...
template<int BlockDim>
void SparseSurfelFusion::ApplySpMVBinBlockCSR<BlockDim>::ApplySpMVTextured(
...
template<int BlockDim>
void SparseSurfelFusion::ApplySpMVBinBlockCSR<BlockDim>::InitResidual(
	...
	//Sanity check
	FUNCTION_CHECK_EQ(x_init.Size(), matrix_size_);
	FUNCTION_CHECK_EQ(b.Size(), matrix_size_);
	FUNCTION_CHECK_EQ(residual.Size(), matrix_size_);
...
template<int BlockDim>
void SparseSurfelFusion::ApplySpMVBinBlockCSR<BlockDim>::InitResidualTextured(
	cudaTextureObject_t x_init,
	DeviceArrayView<float> b,
	DeviceArrayHandle<float> residual,
	cudaStream_t stream
