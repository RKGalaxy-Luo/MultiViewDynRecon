...
#include "jtj_block_jacobian.cuh"
...
#pragma unroll
			...
			for (auto iter = threadIdx.x; iter < padded_term_size; iter += warp_size) {
				...
				if (iter < term_size) {
					...
					case TermType::CrossViewCorr:
					{
						float term_jacobian[18] = { 0 };
						float nodepair_weight = 0;
						computeFeatureJtJBlockJacobian(term2jacobian.cross_corr_term, encoded_pair, typed_term_idx, term_jacobian, &nodepair_weight);
						fillChannelledJtJToSharedBlock(term_jacobian, shared_blks, constants.CrossViewCorrSquared() * nodepair_weight);
					}
					break;
					default:
						term_valid = false;
		...
		__global__ void memoryPerformanceTestKernel(
			DeviceArrayHandle<float> jtj_blks
		) {
			const auto nodepair_idx = blockIdx.x;
			//Write to output
			for (auto iter = threadIdx.x; iter < jtj_blk_size; iter += warp_size)
...
void SparseSurfelFusion::JtJMaterializer::computeNonDiagonalBlocks(cudaStream_t stream)
{
	//Correct the size of node pairs
	const auto num_nodepairs = m_nodepair2term_map.encoded_nodepair.Size();
	FUNCTION_CHECK_EQ(num_nodepairs, m_nodepair2term_map.nodepair_term_range.Size());
...
void SparseSurfelFusion::JtJMaterializer::BuildMaterializedJtJNondiagonalBlocksGlobalIteration(cudaStream_t stream) {
	//Correct the size of node pairs
	const auto num_nodepairs = m_nodepair2term_map.encoded_nodepair.Size();
	FUNCTION_CHECK_EQ(num_nodepairs, m_nodepair2term_map.nodepair_term_range.Size());
	m_nondiag_blks.ResizeArrayOrException(num_nodepairs * device::jtj_blk_size);
