...
void SparseSurfelFusion::KeyValueSort<KeyT, ValueT>::Sort(const DeviceArray<KeyT>& key_in, const DeviceArray<ValueT>& value_in, cudaStream_t stream, int end_bit, bool debug_sync)
{
    ...
    cub::DeviceRadixSort::SortPairs<KeyT, ValueT>(m_temp_storage.ptr(), required_temp_bytes, key_in.ptr(), valid_sorted_key.ptr(), value_in.ptr(), valid_sorted_value.ptr(), (int)key_in.size(), 0, end_bit, stream, debug_sync);
}
